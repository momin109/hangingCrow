...
export class BettingService {
  ...
  private readonly ODDS_FREEZE_SECONDS = parseInt(
    process.env.ODDS_FREEZE_SECONDS || "5",
  );
   ...
   */
  private async createOddsSnapshot(
    marketId: string,
    odds: number,
    fancyId?: number,
    ...
    const hash = crypto
      .createHash("sha256")
      .update(JSON.stringify(oddsData))
      .digest("hex");
  ...
  async placeBet(betData: {
    ...
    this.logger.log(
      `Placing bet for user ${betData.userId}, stake: ${betData.stake}`,
    );

    const isLocked = await this.lockService.isUserOrAncestorLocked(
      betData.userId,
    );
    if (isLocked) {
    ...
    if (user?.parent) {
      ...
      if (
        agentExposure.totalExposure + potentialWin >
        this.MAX_AGENT_EXPOSURE
      ) {
        ...
        throw new ForbiddenException(
          "Bet rejected: Agent exposure limit reached.",
        );
      }
    }

    const oddsSnapshot = await this.createOddsSnapshot(
      betData.marketId,
      betData.odds,
    );
    const potentialWin = betData.stake * betData.odds;
  ...
  async rollbackSettlement(betId: string) {
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (!bet) throw new NotFoundException("Bet not found");
      if (bet.status === "PENDING")
        throw new BadRequestException("Bet is not settled");
      ...
      if (bet.status === "WON") payoutToRevert = Number(bet.potentialWin);
        ...
        payoutToRevert = Number(bet.stake) / 2;
      }
      ...
      if (payoutToRevert > 0) {
        ...
        if (Number(wallet.balance) < payoutToRevert) {
          throw new BadRequestException(
            "User has insufficient balance for rollback",
          );
        }
      ...
      // safer fallback: avoid direct compile error if AuditService lacks logAction()
      const audit = this.auditService as any;
      if (typeof audit.logAction === "function") {
        await audit.logAction(bet.userId, "BET_ROLLBACK", "BET", bet.id, {
